...
from domain.memory.models import IMemoryFilters, IMemoryItemDebugInfo, IMemoryItemInfo, IMemoryUserIdentity, \
    IMemoryUser, QARole, \
...
from shared.utils.keyword import compute_keyword_overlap, compute_token_overlap
from shared.utils.text import canonical_token_set
from shared.utils.vectors import vec_to_buf, cos_sim
...
async def query_hsg_memories(query: str, top_k: int = 10, filters: IMemoryFilters | None = None) -> IMemorySearchResult:
    ...
    try:
        ...
        # =========== 第三路召回：摘要召回（L2）===========
        sessions: SessionCollection = await session_ops.session_search(user, query, top_k=effective_k)
...
def extract_sessions_if_necessary(sessions: SessionCollection, filters: IMemoryFilters, memories: list[IMemoryItemInfo],
