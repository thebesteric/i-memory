...
from shared.config.constants import ModelProvider, VectorStoreProvider, EmbedModelProvider
...
class EnvConfig:
    def __init__(self, env_file_path: str = f"{pyrootutils.find_root()}/.env"):
        ...
        self.REDIS_USER = env_helper.get("REDIS_USER", "default")
        self.REDIS_PASSWORD = env_helper.get("REDIS_PASSWORD", "")
        ...
        # 自动衰减循环间隔（秒）
        self.DECAY_INTERVAL_SECONDS = env_helper.get("IM_DECAY_INTERVAL_SECONDS", TimeUnit.HOURS.to_seconds(1))
        ...
        # 图构建循环间隔（秒）
        self.GRAPH_BUILD_INTERVAL_SECONDS = env_helper.get("IM_GRAPH_BUILD_INTERVAL_SECONDS", TimeUnit.HOURS.to_seconds(2))
        # 图执行至少记忆数量
